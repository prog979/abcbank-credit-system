	...
	public List getClientsByFIO(String client) throws SQLException {
		...
		try {
			...
			clients = session.createQuery(
					"SELECT client FROM Client client WHERE lower(client.name) like concat('%', :client, '%') OR lower(client.secname) like concat('%', :client, '%') OR lower(client.patronymic) like concat('%', :client, '%')")
